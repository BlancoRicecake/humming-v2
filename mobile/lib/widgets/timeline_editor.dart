...
const double _addTrackRowH = 58; // 트랙 목록 맨 아래 추가 행
...
// ─── 스냅 ──────────────────────────────────────────────────────────────
  ...
  List<Widget> _laneRows() {
    ...
    rows.add(_addTrackRow());
    return rows;
  }

  Widget _addTrackRow() {
    return Padding(
      padding: const EdgeInsets.fromLTRB(0, 4, 0, _laneGap),
      child: GestureDetector(
        behavior: HitTestBehavior.opaque,
        onTap: widget.onAddTrack,
        child: Container(
          height: _addTrackRowH - _laneGap,
          decoration: BoxDecoration(
          ...
          child: Row(
            ...
            children: [
              Container(
                ...
                child: const Icon(Icons.add, size: 17, color: AppColors.lime),
              ),
              const SizedBox(width: 8),
              Text(
                L10n.of(context).addTrackTitle,
