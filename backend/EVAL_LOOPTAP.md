...
## Baseline (pYIN, autocorrelation tempo fit) — 2026-06

| metric | dev 30 | test 30 |
|---|---|---|
| Layer1 note_f1 / onset_f1 | 0.037 / 0.473 | 0.043 / 0.446 |
| app_step_f1 (tol=0) | 0.527 | 0.703 |
| app_note_f1 (tol=0) | 0.454 | 0.628 |
| **app_note_f1 (±1)** | **0.864** | **0.870** |
| **app_step_f1 (±1)** | **0.959** | **0.967** |
| app_pitch_acc | 0.863 | 0.893 |
| ceiling survival / inkey | 0.999 / 0.966 | 0.996 / 0.972 |

**Read:** the engine→app conversion preserves the melody well (±1 note ~0.87); the
in-key ladder snap recovers the raw engine pitch error (Layer-1 MAE ~2.1 st →
app pitch acc ~0.88). tol=0 step is dominated by free-rhythm drift, not real-app.
...
- Retrain `learned_pitch_correction` on HumTrans for the residual ~12% pitch errors.
