...
| ✅ | 5-6 내보내기 = 재생 결과와 일치 | LoopTap 에서 해결. 추가 레인 볼륨/악기 불일치는 2026-09-03 수정 (audit C8/C10) |
| ✅ | 5-1 마이크 권한 회복 동선 ✅(2026-09-03, 설정 열기 버튼) · 5-2 / 5-3 은 구 세로형 앱(recording_screen, KEY 카드)의 항목 — LoopTap 에디터에는 해당 화면·용어("신뢰도 · high", "보정됨")가 없음. 관련 ARB 키(keyConfidence 등)는 미사용 잔재. 2026-09-07 확인 후 종결 |
| 낮음 | 4-B 그리드 시트(wheel로 사실상 대체) | 필요 시 보조 액션으로만 |
| ✅ | 5-4 Done 버튼 동작 | LoopTap 은 뒤로가기 = 저장. 시스템 back/edge-swipe 도 2026-09-03 부터 동일 (PopScope) |
