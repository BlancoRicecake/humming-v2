...
`docs/legal/` 의 약관·개인정보처리방침·환불정책이 개정되었다 (미구현이던 클라우드 동기화 조항 삭제, 시행일 **2026-10-20**).

약관 제3조에 따라 **시행일 30일 전** 고지가 필요하다. 2026-09-07 결정: 이메일은 보내지 않고 **웹 공지 + 앱 내 공지**로 간다.

- [x] 랜딩(`landing/{terms,privacy,refund}`) 배포 + 상단 공지 배너 — 2026-09-04 게시 완료
- [ ] 앱 내 공지 — 새 빌드의 홈 배너 → 개정 요약·문서 3개 링크 → "확인했어요" 동작 확인. 확인 전까지 배너가 유지되고, 확인 후에도 설정 → 약관 개정 안내에서 다시 열 수 있음. **2026-09-20까지 스토어 출시** 계획 유지
- [ ] 출시가 09-20 을 넘기면 시행일을 출시일 +30일로 옮기고 관련 문서와 앱 공지 날짜·ID 수정 (`docs/legal/notice-2026-09-05.md` 상단 목록)
- [ ] 시행일(2026-10-20) 도달 확인

시행일을 옮기면 `mobile/lib/looptap/widgets/policy_notice.dart`의
`policyEffectiveDate`와 `policyNoticeId`도 함께 바꾼다. 새 ID는 기존 확인
기록과 구분되어 홈 배너를 다시 표시한다. 본문 파일만 번들에 포함하는
것과 실제 사용자에게 배너를 노출하는 것은 별도로 점검한다.
